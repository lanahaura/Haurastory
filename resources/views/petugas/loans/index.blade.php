@extends('petugas.layout', ['title' => 'Data Peminjaman'])

@section('content')
<div class="bg-white rounded-2xl border border-yellow-100 shadow-sm p-4">
    <h2 class="font-bold text-lg mb-3">Data Peminjaman (Masih Dipinjam)</h2>
            ...
            <tbody>
                @forelse($loans as $l)
                    <tr class="border-t">
                        ...
                        <td class="p-2">
                            @if($l->status === 'pending_borrow')
                                <div class="flex items-center gap-1">
                                    <form method="POST" action="{{ route('petugas.loans.approve-borrow', $l) }}">
                                        @csrf
                                        <button class="px-2 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-xs font-bold">Setuju</button>
                                    </form>
                                    <form method="POST" action="{{ route('petugas.loans.reject-borrow', $l) }}">
                                        @csrf
                                        <button class="px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded text-xs font-bold">Tolak</button>
                                    </form>
                                </div>
                            @elseif($l->status === 'borrowed')
                                <span class="px-2 py-1 rounded-full bg-yellow-200 font-semibold text-yellow-800 text-xs">Dipinjam</span>
                            @elseif($l->status === 'rejected_borrow')
                                <span class="px-2 py-1 rounded-full bg-red-200 font-semibold text-red-800 text-xs">Ditolak</span>
                            @else
                ...
                @empty
                    <tr><td class="p-3" colspan="6">Belum ada peminjaman.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
