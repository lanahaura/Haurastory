...
<div class="max-w-7xl mx-auto p-4 md:p-6">
    ...
    <div class="bg-white border border-yellow-100 rounded-2xl px-5 py-4 shadow-sm flex items-center justify-between">
        <div class="font-extrabold text-lg">🧑‍💼 Petugas Perpustakaan</div>
        <div class="text-sm text-gray-600">Akses: <span class="font-semibold">/petugas</span></div>
    </div>
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <aside class="md:col-span-3">
            <div class="bg-white border border-yellow-100 rounded-2xl p-3 shadow-sm">
                ...
                <div class="space-y-2">
                    <a class="{{ $linkClass(request()->routeIs('petugas.dashboard')) }}"
                       href="{{ route('petugas.dashboard') }}">Dashboard</a>

                    <a class="{{ $linkClass(request()->routeIs('petugas.books.*')) }}"
                       href="{{ route('petugas.books.index') }}">Data Buku</a>

                    <a class="{{ $linkClass(request()->routeIs('petugas.loans.*')) }}"
                       href="{{ route('petugas.loans.index') }}">Data Peminjaman</a>

                    <a class="{{ $linkClass(request()->routeIs('petugas.returns.*')) }}"
                       href="{{ route('petugas.returns.index') }}">Data Pengembalian</a>

                    <a class="{{ $linkClass(request()->routeIs('petugas.reports.*')) }}"
                       href="{{ route('petugas.reports.index') }}">Laporan</a>
                </div>
            </div>
        </aside>
