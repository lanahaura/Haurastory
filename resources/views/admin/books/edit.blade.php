@extends('admin.layout', ['title' => 'Edit Buku'])
...
<div class="bg-white rounded-2xl border border-yellow-100 shadow-sm p-4 max-w-2xl">
    ...
    <form method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data" class="grid gap-3">
        @csrf
        @method('PUT')
    ...
    @error('cover') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.books.index') }}" class="px-4 py-2 rounded-xl bg-yellow-200 hover:bg-yellow-300 font-bold">Batal</a>
        </div>
    </form>
</div>
@endsection
