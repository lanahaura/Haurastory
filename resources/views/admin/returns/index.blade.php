@extends('admin.layout', ['title' => 'Data Pengembalian'])
                        ...
                        <td class="p-2">
                            @if($l->status === 'pending_return')
                                <div class="flex items-center gap-1">
                                    <form method="POST" action="{{ route('admin.returns.approve-return', $l) }}">
                                        @csrf
                                        <button class="px-2 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-xs font-bold">Setuju</button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.returns.reject-return', $l) }}">
                                        @csrf
